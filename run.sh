...
# Argo Setup
#########################
kubectl apply -n ns-mycicd -f https://raw.githubusercontent.com/argoproj/argo-cd/stable/manifests/install.yaml
...
# kubectl wait --for=create secret/argocd-initial-admin-secret -n ns-mycicd --timeout=30s
sleep 5
echo "**** 5secs wait is completed"

kubectl port-forward svc/argocd-server -n ns-mycicd 8090:443 &
echo "**** Port forwarded argo service to 8090"

kubectl -n ns-mycicd get secret argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 --decode && echo
